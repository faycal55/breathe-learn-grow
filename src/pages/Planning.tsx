...
type PlanResponse = { plan: PlanEntry[]; notes?: string };
...
export default function Planning() {
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    try {
      const availability = {
        work: [
          { day: "Mon", start: values.workWeekStart, end: values.workWeekEnd },
          { day: "Tue", start: values.workWeekStart, end: values.workWeekEnd },
          { day: "Wed", start: values.workWeekStart, end: values.workWeekEnd },
          { day: "Thu", start: values.workWeekStart, end: values.workWeekEnd },
          { day: "Fri", start: values.workWeekStart, end: values.workWeekEnd },
          ...(values.weekendWork
            ? [
                { day: "Sat", start: values.weekendStart, end: values.weekendEnd },
                { day: "Sun", start: values.weekendStart, end: values.weekendEnd },
              ]
            : []),
        ],
        sleep: { start: values.sleepStart, end: values.sleepEnd },
      ...
      console.error(e);
      toast({ title: "Erreur", description: e?.message || "Impossible de générer le planning", variant: "destructive" });
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <header>
        <h1 className="text-2xl font-semibold">Planning personnalisé (IA)</h1>
        <p className="text-sm text-muted-foreground mt-1">
          « Je peux t’offrir un soutien fondé sur des études. Je ne remplace pas un professionnel de santé. En cas d’urgence, appelle le 112 (3114 en France). »
        </p>
      </header>
      ...
      <section className="grid lg:grid-cols-3 gap-6">
            ...
            <Form {...form}>
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <FormField name="workWeekStart" control={form.control} render={({ field }) => (
                    <FormItem>
                      <FormLabel>Travail (Lun-Ven) début</FormLabel>
                      <FormControl><Input type="time" {...field} /></FormControl>
                    </FormItem>
                  )} />
                  <FormField name="workWeekEnd" control={form.control} render={({ field }) => (
                    <FormItem>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <FormField name="sleepStart" control={form.control} render={({ field }) => (
                    <FormItem>
                      <FormLabel>Sommeil début</FormLabel>
                      <FormControl><Input type="time" {...field} /></FormControl>
                    </FormItem>
                  )} />
                  <FormField name="sleepEnd" control={form.control} render={({ field }) => (
                    <FormItem>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <FormField name="weekendWork" control={form.control} render={({ field }) => (
                    <FormItem>
                ...
                {form.watch("weekendWork") && (
                  <div className="grid grid-cols-2 gap-3">
                    <FormField name="weekendStart" control={form.control} render={({ field }) => (
                      <FormItem>
                        <FormLabel>Week-end début</FormLabel>
                        <FormControl><Input type="time" {...field} /></FormControl>
                      </FormItem>
                    )} />
                    <FormField name="weekendEnd" control={form.control} render={({ field }) => (
                      <FormItem>
                ...
                <FormField name="commitments" control={form.control} render={({ field }) => (
                  <FormItem>
        ...
        <div className="lg:col-span-2 space-y-4">
          <Card>
            ...
            <CardContent>
              {!plan ? (
                <p className="text-sm text-muted-foreground">Remplissez le formulaire puis générez votre planning.</p>
              ) : (
                <div className="space-y-6">
                    ...
                    <div key={date} className="border rounded-lg overflow-hidden">
                      ...
                      <div className="divide-y">
                        {items
                          ...
                          .map((it, idx) => (
                            <div key={idx} className="grid grid-cols-[80px_1fr] gap-3 px-4 py-3">
                              ...
                              <div>
                                ...
                                {it.rationale && (
                                  <div className="text-xs mt-1">{it.rationale}</div>
                                )}
                                {it.safety_note && (
                                  <div className="text-xs mt-1 text-destructive">Sécurité: {it.safety_note}</div>
                                )}
                              </div>
                            </div>
                          ))}
